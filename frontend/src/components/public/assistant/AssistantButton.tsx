...
import { PointerEvent, MouseEvent, useEffect, useRef, useState } from "react";
...
export default function AssistantButton({ onClick, label = "Shree AI" }: AssistantButtonProps) {
  ...
  const clamp = (x: number, y: number): Position => {
    ...
    return {
      ...
      y: Math.min(Math.max(68, y), Math.max(68, window.innerHeight - s - 8)),
    };
  };
  ...
  const onPointerUp = (event: PointerEvent<HTMLButtonElement>) => {
    ...
    if (!moved.current) {
      // Opening on the next frame prevents the mobile browser's follow-up click
      // from landing on a link/page element underneath the button.
  ...
  const handleClick = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    // Keeps the assistant accessible to keyboard users.
    onClick?.();
  };
  ...
  return (
    <button
      ...
      className="fixed z-[100] grid h-16 w-16 touch-none place-items-center rounded-full border-2 border-[#95CCDD]/65 bg-[#0B1220]/95 p-1.5 shadow-[0_16px_55px_rgba(0,0,0,.5),0_0_34px_rgba(66,116,217,.25)] backdrop-blur-2xl transition-[box-shadow,border-color,transform] duration-200 hover:scale-105 hover:border-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f58a1f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1220] sm:h-[84px] sm:w-[84px]"
    >
